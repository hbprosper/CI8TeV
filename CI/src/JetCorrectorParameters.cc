...
void 
JetCorrectorParametersCollection::getSections(std::string inputFile,
					      std::vector<std::string>& 
					      outputs )
{
  outputs.clear();
...
JetCorrectorParameters const & 
JetCorrectorParametersCollection::operator[]( key_type k ) const {
  ...
  {
    std::cout << "JetCorrectorParameters - cannot find key " << std::endl;
    exit(0);
...
JetCorrectorParametersCollection::findKey( std::string const & label ) const {
  ...
  {
    std::cout << "JetCorrectorParameters - cannot find label " << label 
	      << std::endl;
    exit(0);
